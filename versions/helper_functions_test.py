...
def list_words_of_length(active, length, unique_words_filename):
    if not active:
        print('list_words_of_length not active')
        return []
...
# 6.
def find_words_ending_with_from_file(suffix, filename, target_filename, print_frequencies=True):
    ...
    with open(target_file_path, 'w', encoding='utf-8') as target_file:
        make_line = len(target_file_path)
        extra_length = 14
        ...
        print(f"File created: {target_file_path}")
        print((make_line + extra_length) * "-")
...
def find_words_in_frequencies(active, words, frequencies_filepath, sort_by='alphabetical', search_mode='exact', frequency=None, output_filepath=None):
    """
    ...
    """
    if not active:
        print("find_words_in_frequencies function is not enabled.")
    ...
    if not words and frequency is None:
        print("find_words_in_frequencies: Word search list is empty and no frequency specified!")
    ...
    try:
        ...
        print(f"File not found: {frequencies_filepath}")
        return f"File not found: {frequencies_filepath}"
    except ValueError:
        print(f"Error reading file: {frequencies_filepath}")
    ...
    if frequency is not None:
        ...
        if search_mode == 'exact':
            found_words = {word: word_frequencies[word] for word in words if word in word_frequencies}
        elif search_mode == 'partial':
            for word in words:
                for key in word_frequencies:
                    if word in key:
                        found_words[key] = word_frequencies[key]
        else:
    ...
    # Create the message
    if not sorted_words:
        print(f"No words found using words: {words}.")
    ...
    if frequency is not None and output_filepath:
        ...
        print(f"Words with frequency {frequency} written to {output_filepath}")
